<?session_start();
	...
	if($job_id<>0){
		$result_type = 1;
?>
	<center> พบข้อมูลอาชีพ/ประเภทธุรกิจ - เสี่ยงสูง &nbsp; <strong><font color='red'><u><?=$job_search ?></u></font></strong></center>
<?
	} // end if job_id <> 0 
	...
	if($obj_db->num_rows()>0){
		$result_type = 1;
?>
	<center> พบข้อมูลประเทศ - เสี่ยงสูง &nbsp; <strong><font color='red'><u><?=$country_search ?></u></font></strong></center>
<?
	} // end if obj_db  ประเทศความเสี่ยง

	// ในกรณีที่ไม่ตรงกับเงื่อนไขอาชีพ หรือ ประเทศ ให้ค้นหาจาก 18 ไฟล์ที่เหลือ  ยกเลิกชั่วคราว 03/03/2557
	// if($result_type==0){
	...
	if(trim($number_search) <>''){				/* หมายเลขอ้างอิงหรือไม่  [ เลขทีบัตร / passport ] */
		...
		if($name_search=='' && $first_name=='' && $epithet=='' && $surname=='' ){
			$temp_match = 'Id';
		} // end if 
...
?>
	<center> ไม่พบข้อมูลรายชื่อหรือหมายเลข ที่มีความเสี่ยง &nbsp; <strong><font color='red'><u><?=$user_online.'-'.$random ?></u></font></strong> &nbsp;&nbsp; วัน-เดือน-ปี <font color='red'><? echo date('d-m-Y') ?></font> &nbsp;&nbsp; เวลา <font color='red'><? echo date('G:i') ?></font></center>
<? }else{ 
		$result_type = 1;
?>
<!-- กรณีที่พบรายการ -->
...
<?
	...
	WHILE($rows = $obj_db ->fetch_row()){ 
		...
		if($temp_match == 'Name'){
			...
			$rows2_len = strlen(trim($rows['3']));		// จำนวนตัวเลขทั้งหมดที่พบ

			$num1 = @substr_count(strtoupper($rows['3']), strtoupper($number_search));
			$sum_cal = strlen($number_search)*$num1;
			$rows['3'] = highlightkeyword($rows['3'], $number_search, 1);			// ข้อความต้นฉบับ , ข้อความค้นหา, รูปแบบที่ 1
			$rows['3'] = str_replace('[#]', '<span style="color: '.'red'.';"><u>', $rows['3'] );
			$rows['3'] = str_replace('[@]', '</u></span>', $rows['3'] );
...
//	} // end if ในกรณีที่ไม่ตรงกับเงื่อนไขอาชีพ หรือ ประเทศ ให้ค้นหาจาก 18 ไฟล์ที่เหลือ  ยกเลิกชั่วคราว 03/03/2557

	// เก็บ log ไว้ออกรายงาน
	$sql = " INSERT INTO [tbl-report]([user_online],[type_online],[date_time],[name_search],[first_name],[epithet]
			,[surname],[number_search],[country_search],[slt_type],[rd_type],[result_type],[job_search],[number_ref],[profile_code],[br_code],[division_name])
			VALUES ('".$user_online."','".$type_online."',GETDATE(),'".$name_search."','".$first_name."','".$epithet."','".$surname."',
			'".$number_search."','".$country_search."','".$slt_type."','".$rd_type."','".$result_type."','".$job_search."','".$user_online.'-'.$random."',".$profile_online.",'".$brcode_online."','".$division_name_online."') ";
	$obj_db->query($sql);
