...
//	$tempName = 'hm-master.csv';
//	$tempFile = 'hm-master.csv';
	...
	$temp_type = '';			// กำหนดค่า
	$check_type = strpos($tempName, 'un');		// ตรวจสอบว่ามีคำว่า un ในไฟล์ที่ส่งมาให้หรือไม่
	if($check_type === false) {
		exit();														// ไม่ใช่ไฟล์ที่ต้องการให้ออกไปเลย
	}else{
		$temp_type = 'UN';		// กำหนดให้เป็น un
	} // end if check_type 

	// ใช้เฉพาะ UN เท่านั้น
	// ปรับปรุงทุกรายการใน un ให้เป็น B ก่อน จากนั้นถ้าเจอรายการใหม่หรืออีกครั้งให้เป็น 3
	...
	if($temp_filename=='address'){
		$sql = " UPDATE [tbl-address] SET [DATA_TYPE]='B' WHERE [DATA_TYPE]='3' ";
		$obj_db->query($sql);
	} // end if 

	if($temp_filename=='birthinfo'){
		$sql = " UPDATE [tbl-birthinfo] SET [DATA_TYPE]='B' WHERE [DATA_TYPE]='3' ";
		$obj_db->query($sql);
	} // end if 

	if($temp_filename=='classification'){
		$sql = " UPDATE [tbl-classification] SET [DATA_TYPE]='B' WHERE [DATA_TYPE]='3' ";
		$obj_db->query($sql);
	} // end if 

	if($temp_filename=='criminal'){
		$sql = " UPDATE [tbl-criminal] SET [DATA_TYPE]='B' WHERE [DATA_TYPE]='3' ";
		$obj_db->query($sql);
	} // end if 

	if($temp_filename=='description'){
		$sql = " UPDATE [tbl-description] SET [DATA_TYPE]='B' WHERE [DATA_TYPE]='3' ";
		$obj_db->query($sql);
	} // end if 

	if($temp_filename=='education'){
		$sql = " UPDATE [tbl-education] SET [DATA_TYPE]='B' WHERE [DATA_TYPE]='3' ";
		$obj_db->query($sql);
	} // end if 

	if($temp_filename=='falsepositve'){
		$sql = " UPDATE [tbl-falsepositve] SET [DATA_TYPE]='B' WHERE [DATA_TYPE]='3' ";
		$obj_db->query($sql);
	} // end if 

	if($temp_filename=='idnumber'){
		$sql = " UPDATE [tbl-idnumber] SET [DATA_TYPE]='B' WHERE [DATA_TYPE]='3' ";
		$obj_db->query($sql);
	} // end if 

	if($temp_filename=='image'){
		$sql = " UPDATE [tbl-image] SET [DATA_TYPE]='B' WHERE [DATA_TYPE]='3' ";
		$obj_db->query($sql);
	} // end if 

	if($temp_filename=='master'){
		$sql = " UPDATE [tbl-master] SET [DATA_TYPE]='B' WHERE [DATA_TYPE]='3' ";
		$obj_db->query($sql);
	} // end if 

	if($temp_filename=='name'){
		$sql = " UPDATE [tbl-name] SET [DATA_TYPE]='B' WHERE [DATA_TYPE]='3' ";
		$obj_db->query($sql);
	} // end if 

	if($temp_filename=='relationship'){
		$sql = " UPDATE [tbl-relationship] SET [DATA_TYPE]='B' WHERE [DATA_TYPE]='3' ";
		$obj_db->query($sql);
	} // end if 

	if($temp_filename=='resident'){
		$sql = " UPDATE [tbl-resident] SET [DATA_TYPE]='B' WHERE [DATA_TYPE]='3' ";
		$obj_db->query($sql);
	} // end if 

	if($temp_filename=='role'){
		$sql = " UPDATE [tbl-role] SET [DATA_TYPE]='B' WHERE [DATA_TYPE]='3' ";
		$obj_db->query($sql);
	} // end if 

	if($temp_filename=='sanction'){
		$sql = " UPDATE [tbl-sanction] SET [DATA_TYPE]='B' WHERE [DATA_TYPE]='3' ";
		$obj_db->query($sql);
	} // end if 

	if($temp_filename=='source'){
		$sql = " UPDATE [tbl-source] SET [DATA_TYPE]='B' WHERE [DATA_TYPE]='3' ";
		$obj_db->query($sql);
	} // end if 

	if($temp_filename=='str'){
		$sql = " UPDATE [tbl-str] SET [DATA_TYPE]='B' WHERE [DATA_TYPE]='3' ";
		$obj_db->query($sql);
	} // end if 

	if($temp_filename=='vessel'){
		$sql = " UPDATE [tbl-vessel] SET [DATA_TYPE]='B' WHERE [DATA_TYPE]='3' ";
		$obj_db->query($sql);
	} // end if 
