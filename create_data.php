<?session_start(); 
	...
	$bck1 = $_POST['bck1'];		//  ถ้า 1 เท่ากับมีเลือก บุคคลธรรมดา
	$bck2 = $_POST['bck2'];		//  ถ้า 1 เท่ากับมีเลือก นิติบุคคล
	$rd = $_POST['rd'];				// ถ้า 1 = windows / 2 = unix
	...
	$obj_db->connect_pep();
		...
		if($bck1==1){
			...
			if (is_writable("download/".$filename)) {
				...
				WHILE($rows = $obj_db ->fetch_row()){ 
					...
					if($rows['0']=='National ID' AND strlen($rows['1'])==13){
						$rows['0'] = '01';
					}else{
						$rows['0'] = $array_export[trim($rows['0'])];
						if($rows['0']==''){
							if(trim($rows['22'])=='0'){						// COMP_CODE  บุคคลธรรมดา
								$rows['0'] = '02';
							} // end if 
						} // end if 
					} // end if 

					if($rows['0']<>'01' AND $rows['0']<>'02'){
						continue;
					} // end if 
					...
					$somecontent = 'D|'.$rows['0'].'|'.$rows['1'].'|'.$rows['2'].'|'.$rows['3'].'|';	// D, รหัสประเภทข้อมูล, ข้อมูลที่ใช้ตรวจสอบ, Entity ID, Tile
					$somecontent.= $rows['4'].'|'.$rows['5'].'|'.$rows['6'].'|'.$rows['7'].'|'.$rows['8'].'|'.$rows['9'].'|';	// First_name, Last_Name, First_Name_E, Mid_Name_E, Last_Name_E
					$somecontent.= $rows['10'].'|'.$rows['11'].'|'.$rows['12'].'|'.$rows['13'].'|'.$rows['14'].'|'.$rows['15'].'|';	// Sex, Address, State, Country, Info_Source, RISK_LEVEL_CODE
					$somecontent.= $rows['16'].'|'.$rows['17'].'|'.$rows['18'].'|'.$rows['19'].'|'.$rows['20'];		// High_Risk_Flag, Create_User, Create_DateTime, Update_User, Update_DateTime Flag
				...
				echo "<center> ข้อมูลบุคคลธรรมดาที่มีความเสี่ยงสูง ".$filename."&nbsp;&nbsp;<a href='download/".$filename ."'>download </a></center>";
			} else {
		...
		if($bck2==1){
			...
			if (is_writable("download/".$filename)) {
				...
				fclose($handle);
				$obj_db->free_result();
				echo "<center> ข้อมูลนิติบุคคลที่มีความเสี่ยงสูง  ".$filename."&nbsp;&nbsp;<a href='download/".$filename ."'>download </a></center>";
			} else {
				echo "The file $filename is not writable";
			} // end if is_writable
		} // end if bck2
